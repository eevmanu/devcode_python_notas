## References

### [Quora](https://en.wikipedia.org/wiki/Quora) Answers

- [Which is the best book for learning python for absolute beginners on their own?](https://www.quora.com/Which-is-the-best-book-for-learning-python-for-absolute-beginners-on-their-own)
- [What is the best online resource to learn Python?](https://www.quora.com/What-is-the-best-online-resource-to-learn-Python)
- [What are the best websites to learn Python?](https://www.quora.com/What-are-the-best-websites-to-learn-Python)
- [What are the best resources for learning Python?](https://www.quora.com/What-are-the-best-resources-for-learning-Python)
- [What are the best learning sites for Python?](https://www.quora.com/What-are-the-best-learning-sites-for-Python)

### Learn More Online

- [Python Course - Codecademy](https://www.codecademy.com/learn/python)
- [Python Class - Google](https://developers.google.com/edu/python/)
- [Learn Python Online - Codementor](https://www.codementor.io/learn-python-online)
- [Learn Python The Hard Way using Python 3](https://learnpythonthehardway.org/python3/)
- [Intermediate Python Programming - Sentdex (youtube playlist)](https://www.youtube.com/playlist?list=PLQVvvaa0QuDfju7ADVp5W1GF9jVhjbX-_)
- [Python Patterns](https://github.com/faif/python-patterns)
- [Learn 90% of Python in 90 Minutes](https://www.slideshare.net/MattHarrison4/learn-90)
- [Py (Duoling for Python - Android app)](https://play.google.com/store/apps/details?id=com.py.learn)

### Tutorials & Examples

- [Python Tutorials](https://pythonspot.com)
- [Python Code Examples - Thinkful](https://github.com/Thinkful/thinkful-python-code-examples)

### Python Style Guide

- [PEP 8](https://www.python.org/dev/peps/pep-0008/)
- [Python Style Guide - Google](https://google.github.io/styleguide/pyguide.html)
- [Code Style](http://docs.python-guide.org/en/latest/writing/style/) - [Kenneth Reitz](https://www.kennethreitz.org/)
- [Elements of Python Style](https://github.com/amontalenti/elements-of-python-style)
- [Python Style Guide - W3resource](http://www.w3resource.com/slides/python-style-guide.php)

### Best Practices

- [A Byte of Python](https://python.swaroopch.com/)
- [The Best of the Best Practices (BOBP) Guide for Python](https://gist.github.com/sloria/7001839)
- [Transforming Code into Beautiful, Idiomatic Python](https://gist.github.com/JeffPaine/6213790)
- [Learning Python from Zero to Hero - Medium post](https://medium.freecodecamp.org/learning-python-from-zero-to-hero-120ea540b567)

### Trainig

- [CodeWars](http://www.codewars.com/)
- [CodeFights](https://codefights.com/)
- [HackerRank](https://www.hackerrank.com/)

### How to: API's

- [How to use APIs with Python - Codecademy](https://www.codecademy.com/courses/python-intermediate-en-6zbLp/0/1?curriculum_id=50ecbb9b71204640240001bf)
- [Learn APIs - Codecademy](https://www.codecademy.com/apis)

### Others

- Python module of the week [Twitter](https://twitter.com/pymotw) / [RSS](https://twitrss.me/twitter_user_to_rss/?user=pymotw)
- [Twitter - Python trending repos](https://twitter.com/pythontrending/)
- [Github - Python trending repos weekly](https://github.com/trending/python?since=weekly)
